...
def generate_trend_chart_bytes(trend_data: dict) -> io.BytesIO:
    ...
    dates = [datetime.strptime(d, "%Y-%m-%d") for d in trend_data.get("dates", [])]
    
    ax.plot(dates, trend_data.get("exchange", []), label="Exchange", color="#1D4ED8", linewidth=1.5)
    ax.plot(dates, trend_data.get("sharepoint", []), label="SharePoint", color="#10B981", linewidth=1.5)
    ax.plot(dates, trend_data.get("onedrive", []), label="OneDrive", color="#F59E0B", linewidth=1.5)
    
    ax.set_title("Active Users 180-Day Trend Breakdown", fontsize=10, fontweight="bold", color="#1E293B", pad=10)
    ax.legend(loc="upper left", fontsize=8, frameon=True, facecolor="#F8FAFC", edgecolor="#E2E8F0")
...
def generate_pa_chart_bytes(pa: dict) -> io.BytesIO:
    ...
    active_values = [active_counts.get(k, 0) for k in labels]
    ...
    ax.bar([pos - width/2 for pos in x], values, width, label="Total (Active + Inactive)", color="#94A3B8")
    ax.bar([pos + width/2 for pos in x], active_values, width, label="Active Scans", color="#2563EB")
    
    ax.set_title("Power Automate Workflow Categories", fontsize=10, fontweight="bold", color="#1E293B", pad=10)
    ax.set_xticks(x)
...
def generate_pdf_report(data: dict, filepath: str):
    ...
    fetched_tabs = data.get("fetched_tabs", [])

    # =========================================================================
    # SECTION: TENANT & IDENTITY
    # =========================================================================
    if "tenant_identity" in fetched_tabs:
        story.append(Paragraph("Tenant & Identity", h1_style))
        story.append(Paragraph("This section outlines directory configurations, verified domains, subscribed licensing plan seats, and Entra ID application integration/sign-in telemetry.", body_style))
        story.append(Spacer(1, 10))
        
        # Subscribed SKUs
        sku_list = data.get("skus", [])
        if sku_list:
            story.append(Paragraph("Subscribed SKUs Inventory", h2_style))
        ...
        if dir_data:
            story.append(Paragraph("Directory Summary", h2_style))
            
            # Org Details
            story.append(Paragraph("<b>Organization Details</b>", body_style))
            story.append(Spacer(1, 4))
            ...
            # Domains
            story.append(Spacer(1, 10))
            story.append(Paragraph("<b>Verified Domains</b>", body_style))
            story.append(Spacer(1, 4))
            ...
            # Groups and Users
            story.append(Spacer(1, 10))
            story.append(Paragraph("<b>Directory Accounts Summary</b>", body_style))
            story.append(Spacer(1, 4))
            ...
            rows_spec = [
                ("Total Users", user_counts.get("total", 0)),
                ("Enabled Users", user_counts.get("enabled", 0)),
                ("Disabled Users", user_counts.get("disabled", 0)),
                ("Guest/External Users", user_counts.get("guest", 0)),
                ("Total Groups", group_counts.get("total", 0)),
                ("Security Groups", group_counts.get("security", 0)),
                ("Mail-Enabled Security Groups", group_counts.get("mailEnabledSecurity", 0)),
                ("Distribution Groups", group_counts.get("distribution", 0)),
                ("Microsoft 365 / Unified Groups", group_counts.get("unified", 0))
            ]
            for label, val in rows_spec:
                dir_table_data.append([
                    Paragraph(label, table_cell_bold),
                    Paragraph(f"{val:,}", table_cell_style)
                ])
            dir_table = Table(dir_table_data, colWidths=[250, 250])
    ...
    # =========================================================================
    # SECTION: COMMUNICATION & STORAGE
    # =========================================================================
    if "comm_storage" in fetched_tabs:
        story.append(Paragraph("Communication & Storage", h1_style))
        story.append(Paragraph("This section outlines email usage, calendar resource configurations, mail security rules, client device support breakdown, inbound/outbound connectors, and document storage usage metrics.", body_style))
        story.append(Spacer(1, 10))

        # Email, Calendar and Contacts
        story.append(Paragraph("Email, Calendar & Contacts", h2_style))
        
        # Mailbox summary
        mailbox = data.get("mailbox", {})
        if mailbox:
            story.append(Paragraph("<b>Mailbox Usage Summary</b>", body_style))
            story.append(Spacer(1, 4))
            mb_table_data = [
                [Paragraph("Telemetry Property", table_cell_header), Paragraph("Scanned Value", table_cell_header)],
                [Paragraph("Total User Mailboxes Scanned", table_cell_bold), Paragraph(f"{mailbox.get('total_mailboxes', 0):,}", table_cell_style)],
                [Paragraph("Total Items count", table_cell_bold), Paragraph(f"{mailbox.get('total_emails', 0):,}", table_cell_style)],
                [Paragraph("Total Storage Consumption", table_cell_bold), Paragraph(mailbox.get("total_storage_formatted", "0.00 Bytes"), table_cell_style)],
                [Paragraph("Shared Mailboxes Scanned", table_cell_bold), Paragraph(f"{mailbox.get('shared_mailboxes_count', 0):,}", table_cell_style)],
                [Paragraph("Public Folders Scanned", table_cell_bold), Paragraph(f"{mailbox.get('public_folders_count', 0):,}", table_cell_style)]
            ]
            mb_table = Table(mb_table_data, colWidths=[220, 280])
        ...
        if calendar:
            story.append(Paragraph("<b>Calendar and Room Resources Telemetry</b>", body_style))
            story.append(Spacer(1, 4))
            cal_table_data = [
                [Paragraph("Telemetry Property", table_cell_header), Paragraph("Scanned Value", table_cell_header)],
                [Paragraph("Room Mailboxes Scanned", table_cell_bold), Paragraph(f"{calendar.get('rooms_count', 0):,}", table_cell_style)],
                [Paragraph("Equipment Mailboxes Scanned", table_cell_bold), Paragraph(f"{calendar.get('equipments_count', 0):,}", table_cell_style)]
            ]
            cal_table = Table(cal_table_data, colWidths=[220, 280])
        ...
        # Integrated Apps
        int_apps = data.get("integrated_apps", [])
        if int_apps:
            story.append(Paragraph("<b>Integrated Apps</b>", body_style))
            story.append(Spacer(1, 4))
            ia_table_data = [[
                Paragraph("App Name", table_cell_header),
            ...
            for item in int_apps[:10]:
                ia_table_data.append([
                    Paragraph(item.get("DisplayName", "-"), table_cell_bold),
                    Paragraph(item.get("PublisherName", "-"), table_cell_style),
                    Paragraph(item.get("StoreAssetStatus", "-"), table_cell_style)
                ])
            ia_table = Table(ia_table_data, colWidths=[200, 200, 100])
        ...
        if connectors:
            story.append(Paragraph("<b>Exchange Connectors</b>", body_style))
            story.append(Spacer(1, 4))
            conn_table_data = [[
                Paragraph("Connector Name", table_cell_header),
                Paragraph("Direction / Type", table_cell_header),
                Paragraph("Status", table_cell_header)
            ]]
            ...
            conn_table = Table(conn_table_data, colWidths=[200, 200, 100])
        ...
        if mail_sec and (mail_sec.get("defender", {}).get("skus") or mail_sec.get("eop", {}).get("skus")):
            story.append(Paragraph("<b>Mail Security Configuration</b>", body_style))
            story.append(Spacer(1, 4))
            ms_table_data = [[
                Paragraph("Security Category", table_cell_header),
        ...
        if email_clients:
            story.append(Paragraph("<b>Email Client Classification</b>", body_style))
            story.append(Spacer(1, 4))
            
            # Outlook Clients Table
            clients_list = email_clients.get("outlook_clients", [])
            if clients_list:
                ec_table_data = [[
                    Paragraph("Outlook Client Platform", table_cell_header),
                    Paragraph("Count", table_cell_header)
                ]]
                for platform, count in clients_list:
                    ec_table_data.append([
                        Paragraph(platform, table_cell_bold),
                        Paragraph(count, table_cell_style)
                    ])
                ec_table = Table(ec_table_data, colWidths=[250, 250])
            ...
            # PST Files Table
            pst_list = email_clients.get("pst_files", [])
            if pst_list:
                story.append(Paragraph("<b>Local PST Files Discovered:</b>", body_style))
                story.append(Spacer(1, 4))
                pst_table_data = [[
                    Paragraph("Mailbox Name", table_cell_header),
                    Paragraph("PST Path", table_cell_header),
                    Paragraph("Size", table_cell_header)
                ]]
                for item in pst_list:
                    pst_table_data.append([
                        Paragraph(item.get("MailboxName", "-"), table_cell_bold),
                        Paragraph(item.get("Path", "-"), table_cell_style),
                        Paragraph(item.get("SizeFormatted", "-"), table_cell_style)
                    ])
                pst_table = Table(pst_table_data, colWidths=[150, 250, 100])
        ...
        story.append(Paragraph("Files & Document Storage", h2_style))
        
        # SharePoint
        sp_data = data.get("sharepoint", {})
        if sp_data:
            story.append(Paragraph("<b>SharePoint Online Summary</b>", body_style))
            story.append(Spacer(1, 4))
            sp_table_data = [
                [Paragraph("SharePoint Property", table_cell_header), Paragraph("Scanned Value", table_cell_header)],
                [Paragraph("Total SharePoint sites scanned", table_cell_bold), Paragraph(f"{sp_data.get('total_sites', 0):,}", table_cell_style)],
                [Paragraph("Total Files count", table_cell_bold), Paragraph(f"{sp_data.get('total_files', 0):,}", table_cell_style)],
                [Paragraph("Total Storage Consumption", table_cell_bold), Paragraph(sp_data.get("total_storage_formatted", "0.00 Bytes"), table_cell_style)],
                [Paragraph("Active Files percentage (180 days)", table_cell_bold), Paragraph(f"{sp_data.get('active_files_pct', 0.0):.2f}%", table_cell_style)]
            ]
            sp_table = Table(sp_table_data, colWidths=[220, 280])
        ...
        # OneDrive
        od_data = data.get("onedrive", {})
        if od_data:
            story.append(Paragraph("<b>OneDrive for Business Summary</b>", body_style))
            story.append(Spacer(1, 4))
            od_table_data = [
                [Paragraph("OneDrive Property", table_cell_header), Paragraph("Scanned Value", table_cell_header)],
                [Paragraph("Total accounts scanned", table_cell_bold), Paragraph(f"{od_data.get('total_accounts', 0):,}", table_cell_style)],
                [Paragraph("Total Files count", table_cell_bold), Paragraph(f"{od_data.get('total_files', 0):,}", table_cell_style)],
                [Paragraph("Total Storage Consumption", table_cell_bold), Paragraph(od_data.get("total_storage_formatted", "0.00 Bytes"), table_cell_style)],
                [Paragraph("Active Files percentage (180 days)", table_cell_bold), Paragraph(f"{od_data.get('active_files_pct', 0.0):.2f}%", table_cell_style)],
                [Paragraph("Sync Client active users percentage", table_cell_bold), Paragraph(f"{od_data.get('sync_users_pct', 0.0):.2f}%", table_cell_style)]
            ]
            od_table = Table(od_table_data, colWidths=[220, 280])
    ...
    # =========================================================================
    # SECTION: APPS & AUTOMATION
    # =========================================================================
    if "apps_automation" in fetched_tabs:
        story.append(Paragraph("Apps & Automation", h1_style))
        story.append(Paragraph("This section outlines user active usage trends across different workloads, desktop app deployment version states, and Power Platform workflows metrics.", body_style))
        story.append(Spacer(1, 10))

        # M365 Apps
        has_apps_data = any([data.get("o365_usage"), data.get("o365_trend"), data.get("m365_apps")])
        if has_apps_data:
            story.append(Paragraph("M365 Desktop Workloads Adoption", h2_style))
            
            # Active users usage table
            usage = data.get("o365_usage", [])
            if usage:
                story.append(Paragraph("<b>Active Workload User Allocations (180 days)</b>", body_style))
                story.append(Spacer(1, 4))
                us_table_data = [[
                    Paragraph("Service Workload", table_cell_header),
                    Paragraph("30d Active", table_cell_header),
                    Paragraph("90d Active", table_cell_header),
                    Paragraph("180d Active", table_cell_header)
                ]]
                for row in usage:
                    if len(row) >= 4:
                ...
                us_table = Table(us_table_data, colWidths=[140, 120, 120, 120])
            ...
            # Trend chart
            trend = data.get("o365_trend", {})
            if trend and trend.get("dates"):
                try:
                    trend_bytes = generate_trend_chart_bytes(trend)
                    if trend_bytes:
                        trend_img = Image(trend_bytes, width=450, height=210)
                        story.append(trend_img)
                        story.append(Spacer(1, 10))
                except Exception as trend_ex:
                    print(f"Failed to generate trend chart for PDF: {trend_ex}")

            # M365 App Usage
            apps = data.get("m365_apps", [])
            if apps:
                story.append(Paragraph("<b>M365 App Version Distribution Summary</b>", body_style))
                story.append(Spacer(1, 4))
                apps_table_data = [[
                    Paragraph("Platform OS", table_cell_header),
                    Paragraph("Client App version distribution", table_cell_header),
                    Paragraph("Active users scanned", table_cell_header)
                ]]
                for row in apps[:10]:
                    if len(row) >= 3:
                ...
                apps_table = Table(apps_table_data, colWidths=[120, 260, 120])
                ...
                if len(apps) > 10:
                    story.append(Paragraph(f"...and {len(apps) - 10} more. See generated CSV reports for full details.", ParagraphStyle('Ital', parent=body_style, fontName='Helvetica-Oblique', textColor=secondary_color)))
            story.append(Spacer(1, 15))

        # Power Platform & Automate Flows Analytics
        pa = data.get("power_automate", {})
        if pa:
            story.append(Paragraph("Power Platform & Automate Flows Analytics", h2_style))
            ...
            # Power Automate Breakdown Chart
            if counts:
                try:
    ...
    # =========================================================================
    if "security_compliance" in fetched_tabs:
        story.append(Paragraph("Security & Compliance", h1_style))
        story.append(Paragraph("This section outlines Purview information protection, data lifecycle retention, conditional access, and Intune mobile apps/device configuration distribution.", body_style))
        story.append(Spacer(1, 10))
        ...
        has_sec_data = any([
            ...
            data.get("dlp_policies"),
            data.get("sensitive_info_types"),
            data.get("service_principals_sso"),
            data.get("conditional_access")
        ])
        if has_sec_data:
            story.append(Paragraph("Data Security & Information Governance", h2_style))
            
            # Sensitivity Labels
            labels = data.get("security_labels", [])
            if labels:
                story.append(Paragraph("<b>Microsoft Purview Sensitivity Labels</b>", body_style))
                story.append(Spacer(1, 4))
                labels_table_data = [[
            ...
            if policies:
                story.append(Paragraph("<b>Microsoft Purview Retention Compliance Policies</b>", body_style))
                story.append(Spacer(1, 4))
                ret_table_data = [[
            ...
            if dlp_policies:
                story.append(Paragraph("<b>Microsoft Purview Data Loss Prevention Policies</b>", body_style))
                story.append(Spacer(1, 4))
                dlp_table_data = [[
                    Paragraph("Policy Name", table_cell_header),
            ...
            if sit_types:
                story.append(Paragraph("<b>Sensitive Information Types</b>", body_style))
                story.append(Spacer(1, 4))
                sit_table_data = [[
            ...
            if sso_apps:
                story.append(Paragraph("<b>Enterprise SAML SSO Apps</b>", body_style))
                story.append(Spacer(1, 4))
                mode_counts = Counter()
                for app in sso_apps:
            ...
            if ca_policies:
                story.append(Paragraph("<b>Conditional Access Policies</b>", body_style))
                story.append(Spacer(1, 4))
                ca_table_data = [[
                    Paragraph("Policy Name", table_cell_header),
        ...
        if intune_data:
            story.append(Paragraph("Microsoft Intune Device Configurations", h2_style))
            ...
            story.append(Paragraph("<b>Device Configurations Summary</b>", body_style))
            story.append(Spacer(1, 4))
            if not table_rows:
