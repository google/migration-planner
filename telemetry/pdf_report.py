...
def generate_pdf_report(data: dict, filepath: str):
    ...
    if not transport_rules:
        ...
        for rule in display_rules:
            ...
            safe_desc = html.escape(str(desc_text)).replace('\n', '<br/>')
            safe_name = html.escape(str(rule.get("Name", "-")))
            rules_table_data.append([
                Paragraph(safe_name, table_cell_bold),
                Paragraph(html.escape(str(rule.get("State", "-"))), table_cell_style),
                Paragraph(html.escape(str(rule.get("Priority", "-"))), table_cell_style),
                Paragraph(html.escape(str(rule.get("Mode", "-"))), table_cell_style),
                Paragraph(safe_desc, small_table_cell_style)
            ])
