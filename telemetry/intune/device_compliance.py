...
"""UI component for Device Compliance Policies telemetry."""
...
usage_logger = logging.getLogger("M365TelemetryAsyncLogger.DeviceComplianceUI")

class DeviceComplianceSubFrame(ctk.CTkFrame):
    """Sub-frame for Device Compliance Policies (Android/iOS)."""

    def __init__(self, master, log_callback, credentials_callback, status_change_callback, semaphore=None, device_type="Android", **kwargs):
    ...
    def build_ui(self):
        ...
        ctk.CTkLabel(self.header_frame, text=f"{self.device_type} Devices", font=FONT_BODY_BOLD, text_color=COLOR_TEXT_MAIN).pack(side="left")
        ...
        self.btn_export = ctk.CTkButton(
            self.header_frame, text=f"Export {self.device_type} Policies", width=220, height=32, corner_radius=16,
        ...
        self.summary_lbl = ctk.CTkLabel(self, text=f"Total Extracted: 0 {self.device_type} Compliance Policies", font=FONT_BODY_MEDIUM, text_color=COLOR_TEXT_SUB)
        
        self.grid_frame = ctk.CTkFrame(self, fg_color=COLOR_SURFACE, border_color=COLOR_OUTLINE_LIGHT, border_width=1, corner_radius=8)
        
        headers = ["Display Name", "Description", "Created Time", "Last Modified", "Version"]
        weights = [2, 3, 2, 2, 1]
    ...
    def trigger_fetch(self, tenant, client_id, client_secret):
        ...
        self.csv_path = os.path.join(reports_dir, f"intune_{self.device_type.lower()}_compliance.csv")

        self._set_loading_state(f"Scanning {self.device_type} Compliance Policies...")
    ...
    def _execute_worker(self, tenant, client_id, client_secret):
        ...
        try:
            run_device_compliance_pipeline(
                ...
                filter_type=f"microsoft.graph.{self.device_type.lower()}CompliancePolicy",
            ...
            if not self.is_cancelled:
                ...
                if os.path.exists(self.csv_path):
                    asyncio.run(import_csv_to_sqlite(self.csv_path, db_path, f"{self.device_type.lower()}_compliance"))
                    
                self.status = "success"
                self.after(0, self._render_success)
        except Exception as e:
            usage_logger.error(f"{self.device_type} compliance fetch error: {e}", exc_info=True)
            self.status = "error"
    ...
    def _load_page(self):
        ...
        try:
            ...
            cursor.execute(f"SELECT COUNT(*) FROM {self.device_type.lower()}_compliance")
            total = cursor.fetchone()[0]
            
            offset = self.page * self.ITEMS_PER_PAGE
            cursor.execute(f"SELECT * FROM {self.device_type.lower()}_compliance LIMIT ? OFFSET ?", (self.ITEMS_PER_PAGE, offset))
            rows = cursor.fetchall()
            
            conn.close()
            return rows, total
        except Exception as e:
            usage_logger.error(f"Error reading SQLite {self.device_type.lower()}_compliance: {e}")
            return [], 0
    ...
    def _update_grid(self):
        ...
        self.summary_lbl.configure(text=f"Total Extracted: {total_count} {self.device_type} Compliance Policies")
        ...
        for row_idx, item in enumerate(page_data, start=1):
            ...
            vals = [
                ...
                item.get("createdDateTime") or "N/A",
                item.get("lastModifiedDateTime") or "N/A",
            ...
            for col_idx, val in enumerate(vals):
                ...
                lbl = ctk.CTkLabel(cell, text=str(val), font=FONT_BODY_MEDIUM, text_color=COLOR_TEXT_MAIN, wraplength=280, justify="left", anchor="w")
    ...
    def _export(self):
        ...
        dest_path = filedialog.asksaveasfilename(
            ...
            title=f"Export {self.device_type} Compliance Policies CSV",
            initialfile=f"M365_Intune_{self.device_type}_Compliance_Policies.csv",
            parent=self
        )
        ...
        try:
            ...
            messagebox.showinfo("Export Successful", f"{self.device_type} Compliance Policies successfully exported to:\n{dest_path}")
        except Exception as e:
