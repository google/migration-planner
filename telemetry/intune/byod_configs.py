...
"""UI component for Mobile BYOD Configurations telemetry."""
...
usage_logger = logging.getLogger("M365TelemetryAsyncLogger.ByodConfigsUI")

class ByodConfigsSubFrame(ctk.CTkFrame):
    """Sub-frame for Mobile BYOD Configurations."""
    ...
    def build_ui(self):
        ...
        ctk.CTkLabel(self.header_frame, text="Mobile BYOD Configurations", font=FONT_BODY_BOLD, text_color=COLOR_TEXT_MAIN).pack(side="left")
        ...
        self.btn_export = ctk.CTkButton(
            self.header_frame, text="Export BYOD Configs", width=180, height=32, corner_radius=16,
        ...
        self.summary_lbl = ctk.CTkLabel(self, text="Total Extracted: 0 BYOD Configurations", font=FONT_BODY_MEDIUM, text_color=COLOR_TEXT_SUB)
    ...
    def trigger_fetch(self, tenant, client_id, client_secret):
        ...
        self.csv_path = os.path.join(reports_dir, "intune_byod_configs.csv")

        self._set_loading_state("Scanning Mobile BYOD Configurations...")
        ...
        try:
            ...
            if not self.is_cancelled:
                ...
                if os.path.exists(self.csv_path):
                    asyncio.run(import_csv_to_sqlite(self.csv_path, db_path, "byod_configs"))
                    
                self.status = "success"
                self.after(0, self._render_success)
        except Exception as e:
            usage_logger.error(f"BYOD configs fetch error: {e}", exc_info=True)
            self.status = "error"
    ...
    def _load_page(self):
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM byod_configs")
            total = cursor.fetchone()[0]
            
            offset = self.page * self.ITEMS_PER_PAGE
            cursor.execute("SELECT * FROM byod_configs LIMIT ? OFFSET ?", (self.ITEMS_PER_PAGE, offset))
            rows = cursor.fetchall()
            
            conn.close()
            return rows, total
        except Exception as e:
            usage_logger.error(f"Error reading SQLite byod_configs: {e}")
            return [], 0
    ...
    def _update_grid(self):
        ...
        self.summary_lbl.configure(text=f"Total Extracted: {total_count} BYOD Configurations")
        ...
        for row_idx, item in enumerate(page_data, start=1):
            ...
            vals = [
                ...
                str(item.get("priority") if item.get("priority") is not None else 0),
                item.get("lastModifiedDateTime") or "N/A",
                item.get("iosRestrictions") or "N/A",
                item.get("windowsMobileRestrictions") or "N/A",
                item.get("androidRestrictions") or "N/A"
            ]
    ...
    def _export(self):
        ...
        dest_path = filedialog.asksaveasfilename(
            ...
            title="Export BYOD Configs CSV",
            initialfile="M365_Intune_BYOD_Configs.csv",
            parent=self
        )
        ...
        try:
            ...
            messagebox.showinfo("Export Successful", f"BYOD Configurations successfully exported to:\n{dest_path}")
        except Exception as e:
