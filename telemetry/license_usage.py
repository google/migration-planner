...
class LicenseUsageTab(ctk.CTkScrollableFrame):
    ...
    def _execute_sku_worker(self, tenant: str, clients: List[str], secrets: List[str]):
        ...
        try:
            manager = TokenManager(tenant, clients, secrets, concurrency=1, retries=self.retries.get(), backoff=self.backoff.get())
            manager.authenticate_all(self.log_msg, required_scopes=["Organization.Read.All", "Directory.Read.All"])

            token_slot = manager.get_valid_token_slot(self.log_msg)
            if token_slot and token_slot.get("token"):
                t = token_slot["token"]
                session = manager.get_session()
                
                async_logger.info("Querying Graph API endpoint for SKUs...")
                response = session.get(f"{GRAPH_BASE_URL}/subscribedSkus", headers={"Authorization": f"Bearer {t}", "ConsistencyLevel": "eventual"}, timeout=30)
                manager.return_token_slot(token_slot)
                ...
                if token_slot: manager.return_token_slot(token_slot)
                raise Exception("Failed returning valid security session vectors for SKUs.")
