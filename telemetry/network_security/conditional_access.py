...
"""UI frame for Conditional Access policies under Network Security."""
...
usage_logger = logging.getLogger("M365TelemetryAsyncLogger.ConditionalAccessUI")

class ConditionalAccessSubFrame(ctk.CTkFrame):
    """Sub-frame for Conditional Access network scope policies."""
    ...
    def build_ui(self):
        ...
        ctk.CTkLabel(self.header_frame, text="2. Conditional Access Policies", font=FONT_HEADER_SMALL, text_color=COLOR_TEXT_MAIN).pack(side="left")
        ...
        self.btn_export = ctk.CTkButton(
            self.header_frame, text="Export Conditional Access", width=200, height=32, corner_radius=16,
    ...
    def trigger_fetch(self, tenant, client_id, client_secret):
        ...
        if os.path.basename(script_dir) == "network_security":
            script_dir = os.path.dirname(script_dir)
        reports_dir = os.path.join(script_dir, "reports", f"{tenant}_{client_id}")
        self.csv_path = os.path.join(reports_dir, "network_conditional_access.csv")

        self._set_loading_state("Scanning Conditional Access network scope...")
    ...
    def _render_error(self, err_msg):
        ...
        for w in self.state_frame.winfo_children(): w.destroy()
        self.state_frame.pack(fill="x", expand=True)
        ctk.CTkLabel(self.state_frame, text=f"✖ {err_msg}", text_color=COLOR_ERROR, font=FONT_BODY_MEDIUM, justify="center", wraplength=700).pack(pady=(15, 5))
    ...
    def _update_grid(self):
        ...
        for row_idx, item in enumerate(page_data, start=1):
            ...
            col_keys = ["name", "state", "target_users", "target_apps", "controls"]
            for col_idx, key in enumerate(col_keys):
                ...
                lbl = ctk.CTkLabel(cell, text=str(item.get(key, "N/A")), font=FONT_BODY_MEDIUM, text_color=COLOR_TEXT_MAIN, wraplength=220, justify="left", anchor="w")
    ...
    def _export(self):
        ...
        dest_path = filedialog.asksaveasfilename(
            ...
            title="Export Conditional Access policies CSV",
            initialfile="M365_CA_Network_Access.csv",
            parent=self
        )
        ...
        try:
            ...
            messagebox.showinfo("Export Successful", f"Conditional Access Policies successfully exported to:\n{dest_path}")
        except Exception as e:
