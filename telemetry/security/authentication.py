...
"""UI component for Conditional Access Policies telemetry."""
...
usage_logger = logging.getLogger("M365TelemetryAsyncLogger.AuthUI")

class AuthenticationSubFrame(ctk.CTkFrame):
    """Sub-frame for Conditional Access Policies."""
    ...
    def build_ui(self):
        ...
        ctk.CTkLabel(self.header_frame, text="Authentication Mechanics (Conditional Access)", font=FONT_BODY_BOLD, text_color=COLOR_TEXT_MAIN).pack(side="left")
        
        self.lbl_link = ctk.CTkLabel(
            self.header_frame, text="Open Microsoft Entra Conditional Access Portal ↗",
            font=FONT_BODY_BOLD, text_color=COLOR_PRIMARY, cursor="hand2"
        )
        self.lbl_link.pack(side="left", anchor="w", padx=(15, 0))
        self.lbl_link.bind("<Button-1>", lambda e: __import__("webbrowser").open("https://portal.azure.com/#view/Microsoft_AAD_IAM/ConditionalAccessBlade/~/Policies"))
        ...
        self.btn_export = ctk.CTkButton(
            self.header_frame, text="Export CA Policies", width=180, height=32, corner_radius=16,
    ...
    def trigger_fetch(self, tenant, client_id, client_secret):
        ...
        self.csv_path = os.path.join(reports_dir, "auth_policies.csv")

        self._set_loading_state("Scanning Conditional Access policies...")
    ...
    def _execute_worker(self, tenant, client_id, client_secret):
        ...
        try:
            ...
            usage_logger.error(f"CA policies fetch error: {e}", exc_info=True)
            self.status = "error"
    ...
    def _render_error(self, err_msg):
        ...
        self.state_frame.pack(fill="x", expand=True)
        display_msg = err_msg
        if "401" in err_msg or "403" in err_msg or "permission" in err_msg.lower() or "unauthorized" in err_msg.lower() or "forbidden" in err_msg.lower() or "policy.read" in err_msg.lower():
    ...
    def _load_page(self):
        ...
        try:
            ...
            usage_logger.error(f"Error reading CA policies CSV: {e}")
    ...
    def _update_grid(self):
        ...
        for offset, policy in enumerate(page_data, start=1):
            ...
            target_users = policy.get("target_users", "N/A")
            target_apps = policy.get("target_apps", "N/A")
            controls = policy.get("controls", "N/A")

            vals = [name, state, target_users, target_apps, controls]
    ...
    def _export(self):
        if not self.csv_path or not os.path.exists(self.csv_path):
            messagebox.showinfo("No Data", "There is no CA policy data to export.", parent=self)
            return
            
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        f = filedialog.asksaveasfilename(
            initialfile=f"auth_policies_{ts}.csv",
        ...
        try:
            ...
            messagebox.showinfo("Export Successful", f"Conditional Access policies exported to:\n{f}", parent=self)
        except Exception as e:
