...
"""UI frame for Exchange Online Transport Rules."""
...
class CsvLazyList:
    """A lazy list-like wrapper around a CSV file to provide O(1) memory slicing and length."""
...
class TransportRulesFrame(ctk.CTkFrame):
    """Self-contained component for Exchange Transport Rules UI."""
    ...
    def _fetch_data(self, tenant, c_id, c_secret):
        ...
        try:
            res = run_transport_rules_pipeline(c_id, c_secret, tenant)
            self.csv_path = res["csv_path"]
