...
class CalendarStatsService:
    ...
    def fetch_room_mailboxes(self) -> list:
        """Triggers scripts/exchange_room_mailboxes.ps1 to query the list of room mailboxes."""
        try:
            cert_path = self.client.locate_certificate()
        except Exception as e:
            raise RuntimeError(f"Failed to locate certificate for authentication: {str(e)}")
        ...
        raw_output = self.client.execute_script("scripts/exchange_room_mailboxes.ps1", args)
        if not raw_output or not raw_output.strip():
            return []
