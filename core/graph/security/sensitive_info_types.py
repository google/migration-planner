...
"""Sensitive Information Types (SIT) PowerShell scanner data pipeline."""
...
def run_sensitive_info_types_pipeline(
    client_id: str,
    client_secret: str,
    tenant_id: str
) -> list:
    """Fetch Sensitive Information Types via PowerShell."""
    ...
    try:
        ...
        logger.info(f"Retrieved primary tenant domain for SIT fetch: {tenant_domain}")
    except Exception as e:
        logger.warning(f"Could not retrieve tenant domain for SIT fetch. Falling back to Tenant ID Guid: {e}")
    finally:
        client.close()
    ...
    return dlp_svc.fetch_sensitive_info_types()
