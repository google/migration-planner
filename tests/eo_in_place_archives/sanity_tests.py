"""Sanity tests for the Exchange Online in-place archive scan.

    python3 -m unittest tests.eo_in_place_archives.sanity_tests

No fixture is required. setUp patches TokenManager.authenticate_all because
the estimator authenticates inside __init__. Without that patch the
constructor sets the stop event and every metric silently comes back as zero.
