...
*   **Estimates Only**: TThe estimations provided by this tool are calculated projections intended for preliminary planning only. Actual migration timelines (ETAs) and wave execution may vary based on network latencies, Microsoft/Google throttling policies, migration configurations, data complexity (e.g., large attachments) and the volume of delta migrations. These figures do not constitute a performance guarantee or a binding service level agreement (SLA).
